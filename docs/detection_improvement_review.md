...
**Structural finding, resolved 2026-09-16:** an animal seen entirely INSIDE the fence previously had no path to `animal_candidate`; the fallback only emitted guard/resident. The later full-corpus measurement found a safe conjunction for the elevated/perched subset: calibrated, fully inside, real daylight, colour-bearing, `blob_count <= 2`, and an implausible ground-plane height. `inside_elevated_animal` is evaluated before the generic metric environment gate. It reaches exactly cam10/17145+17146 (one bird event) and cam05/18788 (another labelled animal) across 16,886 clips, with no labelled-negative category change. This does not claim to solve arbitrary inside animals; it closes the evidenced perched-animal failure mode without broadening the general fallback.
