...
| `rectangular_black_white_balance` | per-frame contours → peak | scans filled rectangular contours for a clipped black+white sensor/decode signature, even if the contour did not become the main track | debug/reporting evidence; never suppresses alone. A terminal reverse seed plus independent persistent onside warmup motion can route the event to guard. |
